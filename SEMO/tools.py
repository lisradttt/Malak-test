        ...
        for served_chat in served_chats:
            if f"{served_chat}" in text:
                ...
                try:
                    ...
                    count += 1
                    text += f"{count}:- Not Found {served_chat}\n"
        if count == 0:
    ...
    if message.chat.id == dev or message.chat.username in OWNER:
        ...
        for served_user in served_users:
            if f"{served_user}" in text:
                ...
                try:
                    user = await client.get_chat(served_user)
                    first_name = user.first_name
                    username = user.username
                    count += 1
                    txt = f"{count}:- User : [{first_name}](https://t.me/{username}) Id : `{served_user}`\n" if username else f"{count}:- User : {first_name} Id : `{served_user}`\n"
                    text += txt
                except Exception:
                    count += 1
                    text += f"{count}:- Not Found {served_user}\n"
        if count == 0:
            await m.edit("الاحصائيات صفر ")
        else:
            try:
