...
async def toggle_communication(client, message):
    ...
    if user in OWNER or message.from_user.id == dev:
        ...
        if text == "تفعيل التواصل":
            if client.me.username not in OFFPV:
                return await message.reply_text("**♪ التواصل مفعل من قبل 💎 .**")
            OFFPV.remove(client.me.username)
            return await message.reply_text("**♪ تم تفعيل التواصل 💎 .**")
        elif text == "تعطيل التواصل":
            if client.me.username in OFFPV:
                return await message.reply_text("**♪ التواصل معطل من قبل 💎 .**")
            OFFPV.append(client.me.username)
            return await message.reply_text("**♪ تم تعطيل التواصل 💎 .**")
    ...
    if message.from_user.id == dev or message.chat.username in OWNER or message.from_user.id == client.me.id:
        if message.reply_to_message:
            ...
            try:
                ...
                await message.reply_text(f"**♪ تم ارسال رسالتك إلى {u.mention} بنجاح 💎 .**")
            except Exception:
                pass
    else:
        try:
...
async def welcome_new_member(client: Client, message):
    try:
        ...
        if message.new_chat_members[0].username == "E_BBC":
            try:
            ...
            return await message.reply_text(
                f"**♪ انضم المطور زومبي للشات 💎 .\n♪ مرحبا بك : @E_BBC 💎 .**"
            )
        dev = await get_dev(bot_username)
        if message.new_chat_members[0].id == dev:
            try:
                await client.promote_chat_member(
                    message.chat.id, message.new_chat_members[0].id, privileges=enums.ChatPrivileges(
                        can_change_info=True,
                        can_invite_users=True,
                        can_delete_messages=True,
                        can_restrict_members=True,
    ...
    if message.chat.id == dev or message.chat.username in OWNER:
        ...
        if os.path.isfile(f"{username}.png"):
            ...
            if not bot.photo:
                ...
                return await client.send_message(message.chat.id, "ѕᴇʟᴇᴄᴛ ʏᴏụʀ ʟᴀɴɡụᴀɡᴇ ♪", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
            photo = bot.photo.big_file_id
            photo = await client.download_media(photo)
            username = client.me.username
            photo = await gen_bot(client, username, photo)
        button = [[InlineKeyboardButton(text="ᴇɴɡʟɪѕʜ 🇺🇲", callback_data=f"english"), InlineKeyboardButton(text="عربي 🇪🇬", callback_data=f"arbic")], [InlineKeyboardButton(text=f"{nn}", user_id=f"{dev}")]]
        await client.send_photo(message.chat.id, photo=photo, caption="الرجاء الضغط علي اللغة اذا كانت اللغة العربية او باللغة الانجلزية\n\nᴘʟᴇᴀѕᴇ ᴄʟɪᴄᴋ ᴏɴ ᴛʜᴇ ʟᴀɴɡụᴀɡᴇ ɪғ ɪᴛ ɪѕ ᴀʀᴀʙɪᴄ ᴏʀ ᴇɴɡʟɪѕʜ", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
...
async def E_BBC(client, message):
    bot_username = client.me.username
    dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("يوسف", ""))
...
nkta = [
    ...
    "ﻣﺮه واﺣﺪ ﻣﺴﻄﻮل ﻣﺎﺷﻰ ﻓﻰ اﻟﺸﺎرع ﻟﻘﻰ مذﻳﻌﻪ ﺑﺘﻘﻮﻟﻪ ﻟﻮ ﺳﻤﺢﺖ ﻓﻴﻦ اﻟﻘﻤﺮ ﻗﺎﻟﻬﺎ اﻫﻮه ﻗﺎﻟﺘﻠﻮ ﻣﺒﺮوك ﻛﺴﺒﺖ ﻋﺸﺮﻳﻦ ﺟﻨﻴﻪ ﻗﺎﻟﻬﺎ ﻓﻰ واﺣﺩ ﺗﺎﻧﻰ ﻫﻨﺎك اﻫﻮه 👻😹",
    ...
    "واحد بيشتكي لصاحبه بيقوله أنا مافيش حد بيحبني ولا يفتكرني أبدًا، ومش عارف أعمل إيه قاله سهلة استلف من الناس فلوس هيسألوا عليك كل يوم 👻😹",
    "ﻣﺮه واﺣﺩ ﻣﺴﻄﻮل ﻣﺎﺷﻰ ﻓﻰ اﻟﺸﺎرع ﻟﻘﻰ مذﻳﻌﻪ ﺑﺘﻘﻮﻟﻪ ﻟﻮ ﺳﻤﺢﺖ ﻓﻴﻦ اﻟﻘﻤﺮ؟ ﻗﺎﻟﻬﺎ اﻫﻮه ﻗﺎﻟﺘﻠﻮ ﻣﺒﺮوك ﻛﺴﺒﺖ ﻋﺸﺮﻳﻦ ﺟﻨﻴﻪ ﻗﺎﻟﻬﺎ ﻓﻰ واﺣﺩ ﺗﺎﻧﻰ ﻫﻨﺎك اﻫﻮه 👻😹",
    ...
    "واحده ست سايقه على الجي بي اي قالها انحرفي قليلًا قلعت الطرحة 👻😹",
    "مرة واحد غبي معاه عربية قديمة جدًا وبيحاول يبيعها وماحدش راضي يشتريها.. راح لصاحبه حكاله المشكلة صاحبه قاله عندي لك فكرة جهنمية هاتخليها تتباع الصبح أنت تجيب علامة مرسيدس وتحطها عليها. بعد أسبوعين صاحبه شافه صدفة قاله بعت العربية ولا لاء؟ قاله انت  مجنون حد يبيع مرسيدس 👻😹",
    "مره واحد بلديتنا كان بيدق مسمار فى الحائط فالمسمار وقع منه فقال له :تعالى ف مجاش, فقال له: تعالي ف مجاش. فراح بلديتنا رامي على المسمار شوية مسامير وقال: هاتوه 👻😹",
    "مرة واحد سال واحدة قالها اسمك اي قالتلو دارين قالها وانا غرفتين وصالة 👻😹",
    "مرة واحد سال واحدة قالها اسمك اي قالتلو دارين قالها وانا غرفتين وصالة 👻😹",
    "مرة واحد مسطول طلب من صاحبه المسطول يمشي على دخان السجارة.. قال له أنت عبيط؟ افرض طفيت السجارة اقع 👻😹",
    " مهندس برمجة اتجوز وخلف بنتين توأم.. سمى واحدة لوجين والتانية Log out 👻😹",
